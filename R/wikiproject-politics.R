## Data analysis and model building for WikiProject Politics
...
data_dir = 'datasets/wikiproject-politics';
config_file = 'politics-config.yaml';
...
## so we're removing those.
proj_full = proj_full[!(talk_page_id %in% c(31681, 3786608, 17670120, 24217345,
                                            3337319, 44696158))];
...
## We have 1252 High-importance articles, and over 2,000 Top-importance
## articles. We can sample 250 for the test set, use 1,000 for the training
## set, and we should be alright. In other words, no need for SMOTE here.
n_samples_test = 40;
n_samples_training = 70;
...
proj_full[talk_page_id %in% proj_full[,
                                      .SD[!(talk_page_id %in% proj_full[is_test == 1]$talk_page_id) & sample(.N, n_samples_training)],
...
## We chose min. nodesize of 2, as it has the lowest deviance
...
proj_pred_reratings = rbind(
  ...
  proj_full[importance_rating == "Low" & gbm_pred == "Top",
            list(talk_page_title, gbm_pred)][sample(.N, 8)],
  ## 8 Top-importance articles were predicted to be High-importance,
  ## so we grab all of them instead of sampling:
  proj_full[importance_rating == "Top" & gbm_pred == "High",
  ...
  proj_full[importance_rating == "Low" & gbm_pred == "Mid",
            list(talk_page_title, gbm_pred)][sample(.N, 8)],
  ## 3 Top-importance articles predicted to be Low, so the other two samples
